...
function activate(code,productIds){
  ...
  var products=Array.isArray(productIds)&&productIds.length?productIds.slice():null;
  var previous=get('subscription',null);
  if(!products&&previous&&Array.isArray(previous.activatedProducts)&&previous.activatedProducts.length) products=previous.activatedProducts.slice();
  var sub={planId:r.plan.id,planDays:r.plan.days,currency:'IRT',startedAt:base,expiresAt:expiresAt,verifiedAt:now,verifiedByCode:true};
  if(products) sub.activatedProducts=products;
  set('subscription',sub);
  return{ok:true,expiresAt:expiresAt,plan:r.plan,subscription:sub};
}
window.LangBlueCore={version:'1.0.0-phase5',session:session,userId:userId,account:account,storage:{get:get,set:set,remove:remove,key:key},subscription:{current:currentSubscription,hasProduct:hasProduct,hasAnyProduct:hasAny,hasAllProducts:hasAll,validateCode:validateCode,activate:activate}};
})(window);
